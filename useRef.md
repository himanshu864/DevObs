...
- Or conversly, we could use useRef to access input by reference. And then transfer state.
...
- **useImperativeHandle** : when forwarding a ref. It can be troublesome to work on different components while trying to understand how refs work.
- That's why to manage forwarded ref inside the same component, and only call it from outside we use this hook
